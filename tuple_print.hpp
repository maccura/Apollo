#ifndef __TUPLE_PRINT_H__
#define __TUPLE_PRINT_H__
...
namespace details
{
	...
	void tuple_print_impl(IndexSeq< Indexes... >, std::tuple<Args...>&& tup)
	{
...
{
	details::tuple_print_impl(typename make_indexes<Args...>::type(), std::tuple<Args...>(tup));
}

template<typename ... Args>
void print(std::tuple<Args...>&& tup)
{
	details::tuple_print_impl(typename make_indexes<Args...>::type(), std::forward<std::tuple<Args...>>(tup));
}
...
void print_impl2(const T& tup, IndexSeq<Indexes...>)
{
	//< hack 
	//< 1. 逗号操作符: d = (a = b, c); a会被赋值b, d会被赋值c, 会按顺序执行逗号前面的表达式,
	//<    最终的值是逗号操作符的最后一个值, 即: d = c
...
{
	//< just for log.
	//< if you got an error message: incomplete type, because instantiating 
	//< template function 'tup' variable is not known at compile - so the const expresion like
